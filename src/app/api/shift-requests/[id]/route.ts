...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (action === 'APPROVED') {
      let otherPendingRequests: { userId: string }[] = [];
      ...
      await prisma.$transaction(async (tx) => {
        ...
        // Fetch and reject all other PENDING requests inside transaction (prevents race condition)
        otherPendingRequests = await tx.shiftRequest.findMany({
          where: {
